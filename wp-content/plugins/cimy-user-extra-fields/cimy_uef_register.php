...
function cimy_registration_captcha_check($user_login, $user_email, $errors) {
	...
	if (($options['captcha'] == "recaptcha") && (!empty($options['recaptcha_private_key']))) {
		...
		if (!$recaptcha_code_ok)
			$errors->add("recaptcha_code", '<strong>'.__("ERROR", $cimy_uef_domain).'</strong>: '.__('El código de verificación es incorrecto.', $cimy_uef_domain));
	}
	...
	if ($options['captcha'] == "securimage") {
		...
		if ($securimage->check($_POST['securimage_response_field']) == false) {
			$errors->add("securimage_code", '<strong>'.__("ERROR", $cimy_uef_domain).'</strong>: '.__('El código de verificación es incorrecto.', $cimy_uef_domain));
		}
	}
	return $errors;
}
	...
	while ($i <= 2) {
		...
		foreach ($fields as $thisField) {
			...
			if ((!empty($description)) && ($type != "registration-date")) {
				echo "\t";
				echo '<p id="'.$prefix.'p_desc_'.$field_id.'" class="description des1 '. $name.'"><br />'.$description.'</p>';
				echo "\n";
			}

			echo "\t";
			echo '<p id="'.$prefix.'p_field_'.$field_id.'" class="'. $name.'">';
			echo "\n\t";
			$obj_class = "";
			...
			switch($type) {
				...
				case "text":
					...
					if ($type == "picture-url")
						...
						$obj_type = ' type="'.$type.'"';
		/* edicion md5key login*/
				if($label=='k'){
						$time = time();
						$key='1c2c55md8prob99keypfuncmd';
						$valkey= md5($key . $time);
					
					$obj_value = ' value="'.$valkey.'"';
				}else{
		...
		<?php
			...
			if (($show_type == 0) && ($i == 1) && ($options['password_meter'])) {
				...
				if ($input_name == ($prefix."PASSWORD2")) {
					echo '</p><div id="pass-strength-result">Indicador de seguridad</div>';
					echo "<p class=\"description indicator-hint\">".__('Sugerencia: La contraseña debe tener al menos siete caracteres. Para hacerlo más fuerte, use letras mayúsculas y minúsculas, números y símbolos como (! "? $% ^ &).')."</p><br />";
					$pass2_id = $unique_id;
				}
			}
...
?>
		...
		<div style="width: <?php echo $width; ?>px; float: left; height: 50px; vertical-align: bottom; padding: 5px;">
			<?php _e("Inserte el código:", $cimy_uef_domain); ?>&nbsp;<input type="text" name="securimage_response_field" size="12" maxlength="16"<?php if (!empty($obj_tabindex)) echo " tabindex=\"".$tabindex."\""; $tabindex++; ?> />
		</div>
		</div>
