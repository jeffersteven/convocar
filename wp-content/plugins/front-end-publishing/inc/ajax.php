...
function fep_delete_posts(){
	try{
		...
		if(!current_user_can('delete_posts', $_POST['post_id']))
		...
		$data['success'] = true;
		$data['message'] = 'Publicaión borrada!';
	}
	catch(Exception $ex){
		$data['success'] = false;
...
function fep_process_form_input(){
	try{
		...
		if($fep_misc['nofollow_body_links'])
			...
			wp_create_category(sanitize_text_field($_POST['post_category']));
			$idcat= category_exists(sanitize_text_field($_POST['post_category']));
		...
		$data['message'] = 'Completo!<br/><a href="#" id="fep-continue-editing">Continuar editando</a>';
	}
	catch(Exception $ex){
		$data['success'] = false;
		$data['message'] = '<h2>Tienes errores. Intenta de nuevo!</h2>'.$ex->getMessage();
	}
	die(json_encode($data));
}
