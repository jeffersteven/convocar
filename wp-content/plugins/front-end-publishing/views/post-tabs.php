...
<div id="fep-posts">
  <div id="fep-message"></div>
	<ul>
		<li><a <?php if($status == 'publish'): ?>class="active"<?php endif; ?> href="?fep_type=publish">Publicaciones</a></li>
	</ul>
	<div id="fep-post-table-container">
		<?php if( !$author_posts->have_posts() ): ?> 
			Tu no tienes <?=(($status=='publish')?'':$status)?> publicaciones.
		<?php else: ?>
			<p>Tu tienes <b class="count"><?=$author_posts->found_posts?></b> <?=(($status=='publish')?'':$status)?> publicaciones.</p>
		<?php endif; ?>
		<table>
			<?php
			...
			<tr id="fep-row-<?=$postid?>" class="fep-row">
				<td><?php the_title(); ?></td>
				<?php if($status=='publish'): ?><td class="fep-fixed-td"><a href="<?php the_permalink(); ?>" title="View Post">Ver</a></td><?php endif; ?>
				<td class="fep-fixed-td"><a href="?fep_action=edit&fep_id=<?=$postid;?><?=(isset($_SERVER['QUERY_STRING'])?'&'.$_SERVER['QUERY_STRING']:'')?>">Editar</a></td>
				<td class="post-delete fep-fixed-td"><img id="fep-loading-img-<?=$postid?>" class="fep-loading-img" src="<?php echo plugins_url( 'static/img/ajax-loading.gif', dirname(__FILE__) ); ?>"><a href="#">Eliminar</a><input type="hidden" class="post-id" value="<?=$postid?>"></td>
			</tr>
			<?php endwhile; ?>
		</table>
		...
		<div class="fep-nav">
			<?php if($new_exist): ?>
				<a class="fep-nav-link fep-nav-link-left" href="?fep_type=<?=$status?>&fep_page=<?=($paged-1)?>">&#10094; Nuevas</a>
			<?php endif; ?>
			<?php if($old_exist): ?>
				<a class="fep-nav-link fep-nav-link-right" href="?fep_type=<?=$status?>&fep_page=<?=($paged+1)?>">Antiguas &#10095;</a>
			<?php endif; ?>
