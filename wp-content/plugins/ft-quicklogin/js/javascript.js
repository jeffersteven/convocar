	...
	$.fn.fqlBox = function(options) {
		...
		_check_for_message = function() {
			if( window.location.hash ) {
				var fql_box_show = false;
				var fql_box_show_links = true;
				var fql_box_message = '';

				if( window.location.hash == "#fql-box-message-login" ) {
					fql_box_show = true;
					fql_box_show_links = false;
					fql_box_message = 'Bienvenid@!';
				} else if( window.location.hash == "#fql-box-message-logout" ) {
					fql_box_show = true;
					fql_box_message = 'Cerraste sesión.';
				} else if( window.location.hash == "#fql-box-message-register" ) {
					fql_box_show = true;
					fql_box_message = 'Gracias por registrarte.';
				} else if( window.location.hash == "#fql-box-message-lostpassword" ) {
					fql_box_show = true;
					fql_box_message = '<strong>Correo enviado!</strong><br />Por favor revisar tu correo. Si tu no lo encuentras revisa también el spam.';
				}

				if( fql_box_show ) {
					if( !fql_box_show_links ) $('#fql-box-message div.fql-box-links', $(settings.boxID)).hide();
					$('#fql-box-message p.message', $(settings.boxID)).html(fql_box_message);
...
(function($) {
	$.fn.fqlNews = function(options) {
		...
		function _run() {
			...
			$('li:eq('+ vars.currentSlide +')', $(settings.newsID)).fadeToggle(300);
			$('li', $(settings.newsID)).hide();
