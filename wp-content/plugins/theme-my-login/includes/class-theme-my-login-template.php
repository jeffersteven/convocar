...
class Theme_My_Login_Template extends Theme_My_Login_Abstract {
	...
	public function the_action_links( $args = '' ) {
		if ( $action_links = $this->get_action_links( $args ) ) {
			...
			foreach ( (array) $action_links as $link ) {
				echo '<li class="' . esc_html( $link['title'] ) . '"><a  href="' . esc_url( $link['url'] ) . '" rel="nofollow">' . esc_html( $link['title'] ) . '</a></li>' . "\n";
			}
			echo '</ul>' . "\n";
		}
	}
	...
	public static function get_action_template_message( $action = '' ) {
		switch ( $action ) {
			case 'register':
				$message = __( 'Registrarse en convocar', 'theme-my-login' );
				break;
			case 'lostpassword':
				$message = __( 'Ingresar nombre de usuario o correo electrónico. Recibirá un link vía email para crear una nueva contraseña.', 'theme-my-login' );
				break;
			case 'resetpass':
				$message = __( 'Ingrese su nueva contraseña.', 'theme-my-login' );
				break;
			default:
				$message = '';
		}
