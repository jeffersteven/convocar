...
function vntd_blog_post_settings(){   
    ...
    //add_meta_box("blog_post_settings", __("Post Thumbnail",'veented_backend'), "vntd_blog_post_settings_config", "post", "normal", "high");
}
	...
	<li <?php comment_class(); ?> id="comment-<?php comment_ID() ?>">
		...
		<div class="comment">
			...
			<div class="comment-text">
				...
				<?php comment_reply_link(array_merge( $args, array('depth' => $depth, 'max_depth' => $args['max_depth'],'reply_text' => '<i class="fa fa-reply"></i> '.__('Reply','vntd_north')))); ?>		
				<!-- Name -->
...
function vntd_blog_post_tags(){
	if(has_tag()){
	?>
	<p class="post-tags"><?php the_tags('<span>'.__('Tags','vntd_north').': </span>', ', ', '<br />'); ?></p>
	<?php
	}
}
...
function vntd_post_meta() {
	...
	<div class="classic-meta-section">
		...
		<span class="vntd-meta-comments"><a href="<?php echo get_permalink($post->ID).'#comments'?>" title="<?php _e('View comments','veented');?>"><i class="fa fa-comments-o"></i> <?php comments_number('0', '1', '%'); ?></a></span>
...
function vntd_blog_post_content() {
	...
	if($bande){
		if((esc_html( $categories[$key]->name )==$grupo)||(get_user_role1()=='administrator')||
			(esc_html( $categories[$key]->name )=='Sin categoría')||(esc_html( $categories[$key]->name )=='admin')||
			(esc_html( $categories[$key]->name )=='Todos')){
			...
			<div class="details">
				...
				<div class="post-info">
					...
					<a href="<?php echo get_permalink($post->ID).'#comments'?>" title="<?php _e('View comments','veented');?>" class="post-item">
						...
						<?php comments_number('0', '1', '%'); echo ' '; _e('Comments','vntd_north'); ?>
					</a>
				</div>
				<!-- End Post Infos -->
				<!-- Post Description -->
		...
		echo 'No puedes ver este contenido.';
		}
	}
	}
}
