...
class Vntd_Widget_Flickr extends WP_Widget {
	...
	function widget( $args, $instance ) {
        ...
        <div class="flickr-badge">
        	<script type="text/javascript" src="http://www.flickr.com/badge_code_v2.gne?count=<?php echo $number ?>&amp;display=<?php echo $display ?>&amp;size=s&amp;layout=x&amp;source=user&amp;user=<?php echo $userid ?>"></script>
        </div> 
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => 'Flickr', 'userid' => '10133335@N08', 'display' => 'latest', 'number' => '8' );
        ...
        <p>
        <label for="<?php echo $this->get_field_id('userid'); ?>">Flickr User ID:</label>
		<input id="<?php echo $this->get_field_id('userid'); ?>" name="<?php echo $this->get_field_name('userid'); ?>" type="text" value="<?php echo $instance['userid']; ?>" style="width:100%;" />
        </p> 
        
        <p>
        <label for="<?php echo $this->get_field_id('display'); ?>">Display type:</label>
        <select id="<?php echo $this->get_field_id('display'); ?>" name="<?php echo $this->get_field_name('display'); ?>">
            <option <?php if($instance['display'] == "Latest") echo 'selected="selected"'; ?>>Latest</option>
            <option <?php if($instance['display'] == "Random") echo 'selected="selected"'; ?>>Random</option>
		</select>
        </p>
